...
const FileTable = ({ files, onViewHistory }: FileTableProps) => {
  ...
  const groupFilesByHash = useCallback((files: BlockchainFile[]) => {
    ...
    files.forEach(file => {
      ...
      group.push(file);
      groups.set(hash, group.sort((a, b) => b.version - a.version)); // Sort by version desc
    });

    return groups;
  }, []);

  const fileGroups = groupFilesByHash(files);
  ...
  const renderFileGroup = (hash: string, groupFiles: BlockchainFile[]) => {
